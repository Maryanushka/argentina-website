				...
				<aside class="tags md:w-1/4 w-full flex flex-col md:pl-12 md:order-2 order-1">
					...
					<ul>
						<li v-for="tag in getCategories" :key="tag">
							<a href="#" class="flex items-center text-blue hover:text-darkBlue py-2 font-light">
...
export default {
	...
	computed: {
		getParentTitle() {
			return this.$store.getters.navigation.filter((el) => el.uid === this.localePath('blog').slice(1, -1) && el.type === 'page')[0].title
		},
		getCategories() {
